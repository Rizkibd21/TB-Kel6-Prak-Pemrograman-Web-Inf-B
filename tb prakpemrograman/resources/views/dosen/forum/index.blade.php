...
<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mb-6">
    ...
    <!-- Create Topic Form -->
    <form action="{{ route('dosen.forum.store', $course->id) }}" method="POST" class="mb-6">
        @csrf
        <div class="mb-2">
            <textarea name="content" rows="3" class="w-full border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Start a new discussion..."></textarea>
        </div>
    ...
    <div class="space-y-4">
        ...
        <div class="border rounded p-4 hover:bg-gray-50 dark:hover:bg-gray-700">
            ...
            <a href="{{ route('dosen.forum.show', [$course->id, $post->id]) }}" class="text-indigo-600 dark:text-indigo-400 text-sm font-bold hover:underline">View Discussion</a>
        </div>
        @endforeach
    </div>
</div>
