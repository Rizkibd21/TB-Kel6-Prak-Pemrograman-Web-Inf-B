...
<div class="mb-8">
    <div class="flex items-center gap-4 mb-2">
        ...
        <h2 class="text-3xl font-extrabold text-gray-800 dark:text-gray-100">Edit Jadwal Perkuliahan</h2>
    </div>
    <p class="text-gray-600 dark:text-gray-400 ml-12">Perbarui jadwal untuk mata kuliah <strong>{{ $schedule->subject->name }}</strong>.</p>
</div>
    ...
    <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="subject_id" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all shadow-sm" required>
                    ...
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ $schedule->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->name }} ({{ $subject->code }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="user_id" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all shadow-sm" required>
                    ...
                    @foreach($lecturers as $lecturer)
                        <option value="{{ $lecturer->id }}" {{ $schedule->user_id == $lecturer->id ? 'selected' : '' }}>{{ $lecturer->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="time" name="start_time" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all shadow-sm" value="{{ $schedule->start_time }}" required>
            ...
            <div>
                ...
                <input type="time" name="end_time" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all shadow-sm" value="{{ $schedule->end_time }}" required>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="room" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all shadow-sm" placeholder="Contoh: R.301 or Lab Komputer" value="{{ $schedule->room }}">
